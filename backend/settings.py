...
- `genres` — ⚠️ КРИТИЧЕСКИ ВАЖНО: список русских названий жанров из списка выше. Это правило применяется ко ВСЕМ жанрам без исключения. Если пользователь ЯВНО указал жанр (например: "комедию", "хочу драму", "боевик", "триллер", "фантастику"), то этот жанр ОБЯЗАТЕЛЕН и должен быть ЕДИНСТВЕННЫМ в списке genres, если пользователь не уточнил дополнительные жанры. НЕ добавляй дополнительные жанры, если пользователь их не просил явно. Примеры: если пользователь просит "комедию" → genres=["комедия"], НЕ ["комедия", "криминал", "драма"]. Если пользователь просит "триллер" → genres=["триллер"], НЕ ["триллер", "ужасы", "криминал"]. Если пользователь выбрал подтип жанра (например, "черная комедия", "психологический триллер", "научная фантастика"), это НЕ означает, что нужно добавлять дополнительные жанры - просто используй основной жанр (например, "комедия", "триллер", "фантастика") и опиши подтип в query. Дополнительные жанры добавляй ТОЛЬКО если пользователь явно их упомянул (например: "комедия с элементами криминала" или "триллер с элементами ужасов").
...
- `genres` — ⚠️ CRITICALLY IMPORTANT: list of English genre names from the list above. This rule applies to ALL genres without exception. If the user EXPLICITLY specified a genre (e.g., "comedy", "I want a drama", "action", "thriller", "sci-fi"), then this genre is MANDATORY and should be the ONLY one in the genres list, unless the user explicitly mentioned additional genres. DO NOT add additional genres if the user did not explicitly request them. Examples: if user asks for "comedy" → genres=["Comedy"], NOT ["Comedy", "Crime", "Drama"]. If user asks for "thriller" → genres=["Thriller"], NOT ["Thriller", "Horror", "Crime"]. If the user chose a genre subtype (e.g., "black comedy", "psychological thriller", "sci-fi"), this does NOT mean you should add additional genres - just use the main genre (e.g., "Comedy", "Thriller", "ScienceFiction") and describe the subtype in the query. Add additional genres ONLY if the user explicitly mentioned them (e.g., "comedy with crime elements" or "thriller with horror elements").
